...
function CalendarioPequeno() {
  ...
  const setCalendarioPequenomes = useUIStore((s) => s.setCalendarioPequenomes);
  const setDiaSelected = useUIStore((s) => s.setDiaSelected);
  const diaSelected = useUIStore((s) => s.diaSelected);
  ...
  function getDiaclass(dia) {
    ...
    const diaslc = diaSelected && dayjs(diaSelected).format(formato);
    if (diaHoy === curDia) {
          ...
          <React.Fragment key={i}>
            {row.map((dia, idx) => (
              <button
                key={idx}
                onClick={() => {
                  setCalendarioPequenomes(currentMesIdx);
                  setDiaSelected(dia.format('YYYY-MM-DD'));
                }}
